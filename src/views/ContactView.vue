            ...
            <div class="flex flex-col gap-5">
              <div v-for="info in contactInfo" :key="info.label" class="flex gap-4 items-start">
                <div class="w-10 h-10 bg-[#bc9536]/10 border border-[#bc9536]/30 flex items-center justify-center shrink-0 text-[#bc9536]">
                  <!-- Uso seguro de componentes para iconos -->
                  <component v-if="info.iconComponent" :is="info.iconComponent" :size="18" :stroke-width="1.5" />
                  <div v-else-if="info.svgIcon" v-html="info.svgIcon" class="flex items-center justify-center"></div>
                  <svg v-else-if="info.svgPath" width="18" height="18" fill="currentColor" viewBox="0 0 24 24" v-bind="info.svgAttrs">
                    <path :d="info.svgPath" />
                  </svg>
                </div>
                <div>
...
const IG_SVG = `<svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M7.75 2h8.5A5.75 5.75 0 0 1 22 7.75v8.5A5.75 5.75 0 0 1 16.25 22h-8.5A5.75 5.75 0 0 1 2 16.25v-8.5A5.75 5.75 0 0 1 7.75 2Zm0 1.5A4.25 4.25 0 0 0 3.5 7.75v8.5A4.25 4.25 0 0 0 7.75 20.5h8.5a4.25 4.25 0 0 0 4.25-4.25v-8.5A4.25 4.25 0 0 0 16.25 3.5Zm4.25 3a5.5 5.5 0 1 1 0 11 5.5 5.5 0 0 1 0-11Zm0 1.5a4 4 0 1 0 0 8 4 4 0 0 0 0-8Zm5.63-2.63a.88.88 0 1 1 0 1.76.88.88 0 0 1 0-1.76Z"/></svg>`
...
const contactInfo = computed(() => [
  {
    ...
    iconComponent: null,
    svgIcon: WA_SVG,
    value: settings.value?.contactInfo?.phone || '+52 (55) 5075-1396',
    href: settings.value?.contactInfo?.phone ? `https://wa.me/${settings.value.contactInfo.phone.replace(/\D/g,'')}` : 'https://wa.me/5215550751396',
  },
  {
    label: 'Instagram',
    iconComponent: null,
    svgIcon: IG_SVG,
    value: settings.value?.contactInfo?.instagram?.split('/').pop() || '@hevca_photoart',
    href: settings.value?.contactInfo?.instagram || 'https://instagram.com/hevca_photoart',
  },
  {
    label: 'Ubicación',
    iconComponent: MapPin,
    svgIcon: null,
    value: settings.value?.contactInfo?.address || 'Ciudad de México, México',
    href: null,
  },
  {
